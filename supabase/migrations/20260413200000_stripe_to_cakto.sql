-- Migração: Stripe → Cakto
-- Renomeia colunas, atualiza RPCs e triggers para usar nomenclatura Cakto.

-- ============================================================
-- 1) profiles: renomear colunas Stripe → Cakto
-- ============================================================

alter table public.profiles
  rename column stripe_customer_id to cakto_customer_email;

alter table public.profiles
  rename column stripe_subscription_id to cakto_order_id;

-- ============================================================
-- 2) subscription_plans: renomear + adicionar checkout_url
-- ============================================================

alter table public.subscription_plans
  rename column stripe_product_id to cakto_product_id;

alter table public.subscription_plans
  rename column stripe_price_id to cakto_offer_id;

alter table public.subscription_plans
  add column if not exists cakto_checkout_url text;

comment on table public.subscription_plans is
  'Catálogo de planos de assinatura sincronizados com Cakto Products/Offers.';

-- ============================================================
-- 3) subscriptions: renomear colunas Stripe → Cakto
-- ============================================================
...
create index if not exists subscriptions_cakto_customer_idx
  on public.subscriptions (cakto_customer_email);

comment on table public.subscriptions is
  'Espelho local das assinaturas Cakto por usuário/tenant.';

-- ============================================================
-- 4) desafios: adicionar colunas Cakto para ofertas dinâmicas
-- ============================================================
...
comment on column public.desafios.cakto_offer_id is
  'ID da oferta Cakto criada dinamicamente para este desafio (null = gratuito).';
comment on column public.desafios.cakto_checkout_url is
  'URL de checkout Cakto para taxa de inscrição deste desafio.';

-- ============================================================
-- 5) Trigger: atualizar profiles_prevent_privilege_escalation
--    para referenciar novos nomes de coluna
-- ============================================================
...
begin
  if tg_op = 'UPDATE' then
    ...
    if new.cakto_customer_email is distinct from old.cakto_customer_email then
      raise exception 'Atualização de cakto_customer_email apenas via regras do servidor';
    end if;
    if new.cakto_order_id is distinct from old.cakto_order_id then
      raise exception 'Atualização de cakto_order_id apenas via regras do servidor';
    end if;
  end if;
  new.updated_at := now();
  return new;
...
-- ============================================================
-- 6) RPC: internal_update_profile_cakto (substitui _stripe)
-- ============================================================

drop function if exists public.internal_update_profile_stripe(uuid, boolean, text, text);

create or replace function public.internal_update_profile_cakto(
  p_user_id uuid,
  p_is_pro boolean,
...
comment on function public.internal_update_profile_cakto(uuid, boolean, text, text) is
  'Atualiza campos Cakto protegidos em profiles (uso exclusivo de Edge Functions via service_role).';

revoke execute on function public.internal_update_profile_cakto(uuid, boolean, text, text) from public;
revoke execute on function public.internal_update_profile_cakto(uuid, boolean, text, text) from authenticated;
...
comment on function public.admin_list_subscriptions(text, uuid, integer, integer) is
  'Lista assinaturas com dados de usuário/tenant/plano para admin (Cakto).';

grant execute on function public.admin_list_subscriptions(text, uuid, integer, integer) to authenticated;

-- ============================================================
-- 8) Engagement segment match: usar cakto_order_id
-- ============================================================
  ...
  select
    ...
    and (
      ...
      or (
        lower(p_plan) = 'free'
        and not p_pr_is_pro
        and coalesce(nullif(trim(p_pr_cakto_order), ''), '') = ''
      )
      or (
        lower(p_plan) = 'paid'
        and (
          p_pr_is_pro
          or coalesce(nullif(trim(p_pr_cakto_order), ''), '') <> ''
        )
      )
    );
$$;
...
-- ============================================================
-- 9) Recriar RPCs de engajamento referenciando cakto_order_id
--    (substituem pr.stripe_subscription_id por pr.cakto_order_id)
-- ============================================================
...
begin
  ...
  where c.checkin_local_date between p_start and p_end
    ...
    and public.admin_engagement_segment_match(
      tn.region, pr.is_pro, pr.cakto_order_id, p_region, p_user_type, p_plan
    );
  ...
  where (p.created_at at time zone 'America/Sao_Paulo')::date between p_start and p_end
    ...
    and public.admin_engagement_segment_match(
      tn.region, p.is_pro, p.cakto_order_id, p_region, p_user_type, p_plan
    );
  ...
  select coalesce(
    (
      ...
      left join (
        ...
        where c.checkin_local_date between p_start and p_end
          and (p_tenant_id is null or c.tenant_id = p_tenant_id)
          and public.admin_engagement_segment_match(tn.region, pr.is_pro, pr.cakto_order_id, p_region, p_user_type, p_plan)
        group by 1
      ) cc on cc.day = s.day
      left join (
        ...
        where (p.created_at at time zone 'America/Sao_Paulo')::date between p_start and p_end
          and (p_tenant_id is null or p.tenant_id = p_tenant_id)
          and public.admin_engagement_segment_match(tn.region, p.is_pro, p.cakto_order_id, p_region, p_user_type, p_plan)
        group by 1
      ) np on np.day = s.day
    ),
    '[]'::jsonb
...
-- ============================================================
-- 10) RPCs desafios: adicionar colunas cakto ao retorno
-- ============================================================
...
returns table (
  ...
  cakto_offer_id text, cakto_checkout_url text,
...
begin
  ...
  select
    ...
    d.cakto_offer_id, d.cakto_checkout_url,
