-- Epic 2 (desafios-ranking-privado): ranking público (não participantes)
-- - Retorna apenas Top 3
-- - Somente enquanto o desafio estiver ativo (sem expor contagem)

drop function if exists public.get_desafio_ranking_public(uuid);

create or replace function public.get_desafio_ranking_public(p_desafio_id uuid)
returns table (
  user_id uuid,
  nome_exibicao text,
...
grant execute on function public.get_desafio_ranking_public(uuid) to authenticated;
