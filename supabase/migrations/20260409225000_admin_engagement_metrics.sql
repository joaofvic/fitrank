-- US-ADM-14: métricas de engajamento para dashboard admin (platform master)
...
begin
  ...
  from public.profiles p
  where (p.created_at at time zone 'utc')::date between p_start and p_end
  ...
  where c.foto_url is not null
    ...
    and (c.photo_reviewed_at at time zone 'utc')::date between p_start and p_end
    and (p_tenant_id is null or c.tenant_id = p_tenant_id);
...
comment on function public.admin_engagement_metrics(date, date, uuid) is
  'KPIs de engajamento e moderação por período/tenant; apenas is_platform_master.';
