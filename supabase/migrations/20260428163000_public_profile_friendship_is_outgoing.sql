-- Perfis públicos: distinguir pedido pending enviado pelo visitante vs recebido (Epic cancelar solicitação C.3).
...
DECLARE
  ...
  v_friendship_requester_id uuid;
  v_friendship_is_outgoing boolean := false;
BEGIN
  ...
  INTO v_friendship_id, v_friendship_status, v_friendship_requester_id
  ...
  v_friendship_is_outgoing :=
    v_friendship_status = 'pending'
    AND v_friendship_requester_id IS NOT NULL
    AND v_friendship_requester_id IS NOT DISTINCT FROM v_caller;
  ...
  RETURN jsonb_build_object(
    ...
    'friendship_is_outgoing', v_friendship_is_outgoing
  );
END;
$$;
